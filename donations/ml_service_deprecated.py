...
class FoodQualityPredictor:
    ...
    def _validate_csv(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        required_cols = self.NUMERICAL_FEATURES + self.CATEGORICAL_FEATURES + ["freshness_level"]
    ...
    def train_model(self, test_size: float = 0.2, random_state: int = 42) -> Dict[str, float]:
        ...
        self.training_metrics = {"accuracy": accuracy, "train_samples": len(X_train), "test_samples": len(X_test)}
    ...
    def _calculate_freshness_score(self, probabilities: np.ndarray, classes: np.ndarray) -> int:
        """
        ...
        class_weights = {"Fresh": 100, "Medium": 50, "Spoiled": 0}
